...
describe("parseArgs", () => {
  ...
  it("parses local generation export and import commands", () => {
    ...
    expect(
      ...
      command: "compile-generated-batch",
      options: { plan: "plan.json", responses: "responses.json", output: "batch.json" }
    });
    expect(
      parseArgs([
        "attach-batch-images",
        ...
        "images.json",
        "--output",
        "batch-with-images.json"
      ])
    ).toEqual({
      command: "attach-batch-images",
      options: { manifest: "batch.json", images: "images.json", output: "batch-with-images.json" }
    });
    expect(
      parseArgs([
    ...
    expect(
      parseArgs([
        ...
        "sources.json",
        "--output",
        "batch-with-sources.json"
      ])
    ).toEqual({
      command: "attach-batch-sources",
      options: {
        manifest: "batch.json",
        sources: "sources.json",
        output: "batch-with-sources.json"
      }
    });
    expect(
      parseArgs([
    ...
    expect(
      ...
      command: "prepare-content-audit-retry",
      options: {
        manifest: "content-batch.json",
        audit: "content-audit.json",
        output: "retry-batch.json"
      }
    });
    expect(
      parseArgs([
      ...
      command: "prepare-content-remediation-package",
      options: {
        manifest: "content-batch.json",
        audit: "content-audit.json",
  ...
  it("parses OpenAI generation estimate and execution commands", () => {
    expect(parseArgs(["estimate-openai-generation", "--package", "package.json"])).toEqual({
      command: "estimate-openai-generation",
    ...
    expect(
      ...
      command: "generate-openai-articles",
      options: {
  ...
  it("parses a schedule batch inspection command", () => {
    expect(parseArgs(["inspect-schedule-batch", "--batch", "schedule-batch-1"])).toEqual({
      command: "inspect-schedule-batch",
  ...
  it("parses a campaign validation command", () => {
    expect(parseArgs(["validate-campaign", "--manifest", "campaign.json"])).toEqual({
...
describe("commandRequiresDatabase", () => {
  it("keeps read-only entry commands independent from the database", () => {
    ...
    expect(commandRequiresDatabase("compile-generated-batch")).toBe(false);
    expect(commandRequiresDatabase("attach-batch-images")).toBe(false);
